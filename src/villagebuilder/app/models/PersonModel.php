...
class PersonModel {
    ...
    public static function guestToMember() {
        try {
            DB::transaction(function() {
                $userId = DB::table('users')
                       ->where('email', Input::get('email'))
                       ->pluck('id');
                $participantId = DB::table('participant')
                       ->where('user_id', $userId)
                       ->pluck('participant_id');
                $guestRelationships = DB::table('guest_friendship')
                       ->where('guest_id', $participantId)
                       ->lists('person_id');
                DB::table('users')
                        ->where('id', $userId)
                        ->update([
                ...
                DB::table('guest')
                        ->where('guest_id', $participantId)
                        ->delete();
                foreach($guestRelationships as $friend) {
